...
import { Recurso, RecursoRequest } from "@/store/types/recurso";
...
export function RecursoForm({ recurso, onSuccess, onCancel }: RecursoFormProps) {
  const [createRecurso, { isLoading: isCreating }] = useCreateRecursoMutation();
  const [updateRecurso, { isLoading: isUpdating }] = useUpdateRecursoMutation();
  ...
  const form = useForm<RecursoFormData>({
    ...
    defaultValues: {
      numeroLegajo: recurso?.legajo || "",
      apellido: recurso?.apellido || "",
      nombre: recurso?.nombre || "",
      tipoDocumento: recurso?.tipoDocumentoId || "",
      numeroDocumento: recurso?.numeroDocumento || "",
      cuit: recurso?.cuil || "",
      fechaNacimiento: recurso?.fechaNacimiento || "",
      sexo: recurso?.sexo || "",
      estadoCivil: recurso?.estadoCivil || "",
      nacionalidad: recurso?.nacionalidad || "",
      email: recurso?.mailPersonal || "",
      telefono: recurso?.telefono || "",
      celular: recurso?.celular || "",
  ...
  const onSubmit = async (data: RecursoFormData) => {
    try {
      const recursoData: Partial<RecursoRequest> = {
      ...
      if (isEditing && recurso?.id) {
        await updateRecurso({ 
          id: recurso.id, 
          data: recursoData 
        }).unwrap();
      } else {
        await createRecurso(recursoData as RecursoRequest).unwrap();
      }

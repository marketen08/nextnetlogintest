...
import { recoverDoneBackupTasks } from '../storage';

// POST /api/tasks/cleanup - Limpiar tareas con estados inválidos
export async function POST() {
  try {
    const result = recoverDoneBackupTasks();
    
    return NextResponse.json({
      success: true,
      message: `Se recuperaron ${result.fixed} tareas con estados inválidos`,
      data: {
        fixed: result.fixed,
    ...
    return NextResponse.json({
      success: false,
      message: 'Error al recuperar tareas',
